...
check_user_name() {
    ...
    if [ $? -eq 0 ];then
        ...
        echo -e "${RED_COLOR}Add lora user !${END_COLOR}"
    fi
}
...
SetUbuntuSourceList(){
		...
		echo "deb-src http://cn.archive.ubuntu.com/ubuntu/ bionic main restricted universe multiverse" >>/etc/apt/sources.list
		...
		echo "deb-src http://cn.archive.ubuntu.com/ubuntu/ bionic-security main restricted universe multiverse" >>/etc/apt/sources.list
		echo "deb-src http://cn.archive.ubuntu.com/ubuntu/ bionic-updates main restricted universe multiverse" >>/etc/apt/sources.list
		echo "deb-src http://cn.archive.ubuntu.com/ubuntu/ bionic-proposed main restricted universe multiverse" >>/etc/apt/sources.list
		echo "deb-src http://cn.archive.ubuntu.com/ubuntu/ bionic-backports main restricted universe multiverse" >>/etc/apt/sources.list
...
tftp_configure() {
    ...
    grep "/home/lora/tftpboot" /etc/default/tftpd-hpa 1>/dev/null
    if [ $? -ne 0 ];then
        sed  -i '$a\TFTP_DIRECTORY="/home/lora/tftpboot"' /etc/default/tftpd-hpa
...
nfs_configure() {
    ...
    grep "/home/lora/" /etc/exports 1>/dev/null
    if [ $? -ne 0 ];then
        sed -i '$a\/home/lora/  *(rw,nohide,insecure,no_subtree_check,async,no_root_squash)' /etc/exports  #todo
    fi
...
do
    ...
    case $ch in
        1)
            ...
            echo -e "${BLUE_COLOR}SAMBA PATH: /home/lora/ ${END_COLOR}"
            su $user_name
            ;;
        2)
			SetUbuntuSourceList
